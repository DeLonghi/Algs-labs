...
def init():
    for n in range(1, N):
        ...
        for i in range(5):
            ...
            v = np.copy(V)
            bubble_sort_total += get_timestamp(bubbleSort, v)

            v = np.copy(V)
            quicksort_sort_total += get_timestamp(quicksort, v)

            v = np.copy(V)
            timsort_sort_total += get_timestamp(timSort, v)
...
def init_matrix():
    ...
    for n in range(1, N):
        for i in range(5):
            ...
            t1 = time.time()
            np.matmul(A, B)
            matrix_mul_total += time.time() - t1
        N_for_ATLAS_mat_mul.append(0.00000000455 * n**3)
        y_matrix_mul.append(matrix_mul_total / 5)

# init()
init_matrix()

fig, axes = plt.subplots()
plt.plot(N_const, label = 'Experimental time')
plt.plot(y_const, label = 'Theoretical time')
...
fig, axes = plt.subplots()
plt.plot(N_pow_2, label = 'Experimental time')
plt.plot(y_bubble_sort, label = 'Theoretical time')
axes.set_title("Bubblesort sort")
axes.set_xlabel('n')
axes.set_ylabel('second')
plt.legend()

fig, axes = plt.subplots()
plt.plot(N_log_N, label = 'Experimental time')
plt.plot(y_quicksort_sort, label = 'Theoretical time')
axes.set_title("Quick sort")
axes.set_xlabel('n')
axes.set_ylabel('second')
plt.legend()

plt.plot(N_log_N, label = 'Theoretical time')
plt.plot(y_timsort_sort, label = 'Experimental time')
axes.set_title("Timsort")
axes.set_xlabel('n')
axes.set_ylabel('seconds')
plt.legend()
...
fig, axes = plt.subplots()
plt.plot(N_pow_1, label = 'Experimental time')
plt.plot(y_horner, label = 'Theoretical time')
...
fig, axes = plt.subplots()
plt.plot(N_for_ATLAS_mat_mul, label = 'Theoretical time')
plt.plot(y_matrix_mul, label = 'Experimental time')
axes.set_title("Matrix Multiplication")
axes.set_xlabel('n')
axes.set_ylabel('seconds')
plt.legend()
